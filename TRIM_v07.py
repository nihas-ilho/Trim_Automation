...
def measure_freq2():
    ...
    value = ser.readline().decode().strip()
    ser.write(b'*CLS\n')
    log_text.insert(tk.END, f"[DMM2] Frequency = {value} Hz\n")
    log_text.see(tk.END)
    update_result_display(value, "Hz")
...
def measure_current2():
    ...
    value = ser.readline().decode().strip()
    ser.write(b'*CLS\n')
    log_text.insert(tk.END, f"[DMM2] Current = {value} A\n")
    log_text.see(tk.END)
    update_result_display(value, "A")
...
def led1_search_worker(sample_num):
    ...
    try:
        ...
        #pwr_on()
        root.after(0, lambda: control_arduino_relay(2, "OFF"))
        threading.Event().wait(2.0) # Wait for PSU and relay to stabilize
        ...
        with open(file_path, "a") as f:
            ...
            for i in range(15, 37):  # LED1 코드 범위 축소: 15~37
                ...
                root.after(0, lambda line=log_line: (log_text.insert(tk.END, line), log_text.see(tk.END), f.write(line)))
        ...
        root.after(0, lambda: messagebox.showerror("LED1 Search Error", f"An error occurred: {e}"))
    finally:
        root.after(0, lambda: led1_search_button.config(state=tk.NORMAL))
        if power_supply:
            pwr_off()
...
def led2_search_worker(sample_num):
    ...
    try:
        ...
        with open(file_path, "a") as f:
            ...
            for i in range(15, 37):  # LED2 코드 범위 축소: 15~37
                ...
                if send_pulse_command_internal(5080): # Wait for ACK from pulse command
                    raw_value = _get_fast_current()   # Measure immediately after ACK
        ...
        root.after(0, lambda: messagebox.showerror("LED2 Search Error", f"An error occurred: {e}"))
    finally:
        root.after(0, lambda: led2_search_button.config(state=tk.NORMAL))
        if power_supply:
            pwr_off()
...
def tc_search_worker(sample_num, target_current_amps):
    ...
    try:
        ...
        messagebox.showerror("TC Search Error", f"An error occurred: {e}")
    finally:
...
def run_all_searches_orchestrator(sample_num):
    """BGR -> MCLK -> WCLK -> TC(Method2) -> LED1 -> LED2 순서로 일괄 검색."""
    global last_script_context
    try:
        root.after(0, lambda: run_all_searches_button.config(state=tk.DISABLED))
        ...
        root.after(0, lambda: messagebox.showerror("All Searches Error", f"An unexpected error occurred during all searches:\n{e}"))
    finally:
        root.after(0, lambda: run_all_searches_button.config(state=tk.NORMAL))
        root.after(0, log_text.see, tk.END)
    run_nvm_program ()   
...
def run_all_searches():
    ...
    threading.Thread(target=run_all_searches_orchestrator, args=(sample_num,), daemon=True).start()
   
def run_all_searches_orchestrator2(sample_num):
    """BGR -> MCLK -> WCLK -> TC(Method2) -> LED1 -> LED2 순서로 일괄 검색. (복사본)"""
    global last_script_context
    try:
        root.after(0, lambda: run_all_searches_button2.config(state=tk.DISABLED))
        root.after(0, lambda: log_text.insert(tk.END, f"[INFO] Starting ALL Searches 2 for Sample: {sample_num}...\n"))
        root.after(0, log_text.see, tk.END)
        ...
        for ctx, label, worker_fn, result_fn in sequence:
            root.after(0, lambda l=label: log_text.insert(tk.END, f"[INFO] --- Starting {l} Search (V2) ---\n"))
            last_script_context = ctx
            worker_fn(sample_num)
            if result_fn() == -1:
                root.after(0, lambda l=label: messagebox.showerror("All Searches 2 Failed", f"{l} Search failed. Stopping all searches."))
                return

        root.after(0, lambda: log_text.insert(tk.END, "[INFO] ALL Searches 2 completed successfully!\n"))
    except Exception as e:
        root.after(0, lambda: messagebox.showerror("All Searches 2 Error", f"An unexpected error occurred during all searches (V2):\n{e}"))
    finally:
        root.after(0, lambda: run_all_searches_button2.config(state=tk.NORMAL))
        root.after(0, log_text.see, tk.END)
    run_nvm_program()

def run_all_searches2():
    """Triggers the duplicated 'Run ALL Searches' sequence."""
    controller_connected = (Nordic_mcu and Nordic_mcu.running) or arduino
    if not controller_connected or not multimeter or not multimeter2 or not power_supply:
        messagebox.showerror("Connection Error", "Please connect a Controller, Multimeter (COM5), Multimeter 2 (COM11), and Power Supply for all searches.")
        return
    ...
    threading.Thread(target=run_all_searches_orchestrator2, args=(sample_num,), daemon=True).start()
...
def run_nvm_program():
    ...
    def worker():
        ...
        try:
            ...
            execute_script_from_thread("TRIM_NVM_PGM.txt") # Initial setup script
            time.sleep(0.1)
            ...
            root.after(0, lambda: messagebox.showerror("NVM Program Error", str(e)))
    threading.Thread(target=worker, daemon=True).start()

# def run_nvm_program_led():
#     """Programs the best found trim codes for LED1 and LED2 into NVM."""
#     if -1 in [best_led1_code, best_led2_code]:
#         messagebox.showwarning("Prerequisite Error", "Both LED1 and LED2 searches must be completed first.")
#         return
#     def worker():
#         try:
#             root.after(0, lambda: log_text.insert(tk.END, "[NVM PGM] Starting NVM Programming for LED Trim...\n"))
#             execute_script_from_thread("TRIM_NVM_PGM.txt")
#             time.sleep(0.1)

#             i2c_write(0x3F, 0x04); i2c_write(0x40, best_led1_code); i2c_write(0x41, 0x10); i2c_write(0x41, 0x00)
#             i2c_write(0x3F, 0x05); i2c_write(0x40, 64 + best_led2_code); i2c_write(0x41, 0x10); i2c_write(0x41, 0x00)

#             i2c_write(0x56, 0xA4) # NVM_PGM key
#             i2c_write(0x55, 0xA3) # NVM_PGM_EN key
            
#             time.sleep(0.5)
#             root.after(0, lambda: log_text.insert(tk.END, "[NVM PGM] LED Trim NVM Programming sequence sent.\n"))
#         except Exception as e:
#             root.after(0, lambda: messagebox.showerror("NVM Program Error", str(e)))
#     threading.Thread(target=worker, daemon=True).start()
...
if __name__ == "__main__":
    ...
    # --- Search Sections ---
    headers = ["Search", "Result", "Trim Code", "Action"]
    for col, header in enumerate(headers):
        tk.Label(frame_auto_search, text=header, font=('Helvetica', 9, 'bold')).pack(in_=frame_auto_search, side="top", anchor="w", expand=False if col == 0 else True, fill="x", padx=(10 if col ==0 else 0, 0))
    ...
    #tk.Button(frame_nvm, text="NVM Program (LED1, LED2)", command=run_nvm_program_led).pack(side="left", expand=True, fill="x", padx=2, pady=2)
